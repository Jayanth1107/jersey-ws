...
public class ActivityClient {
	...
	public Activity getActivity(String activityId) {
		...
		// Activity response =
		// target.path("activities/"+activityId).request(MediaType.APPLICATION_JSON).get(Activity.class);
		Response response = target.path("activities/" + activityId).request(MediaType.APPLICATION_JSON)
				.get(Response.class);
		// String response =
		// target.path("activities/"+activityId).request().get(String.class);
		// String response =
		// target.path("activities/"+activityId).request(MediaType.APPLICATION_JSON).get(String.class);
		// return response;
	...
	public List<Activity> getActivities() {
		WebTarget target = client.target("http://localhost:8080/jersey-ws/webapi/");
		// Activity response =
		// target.path("activities").request(MediaType.APPLICATION_JSON).get(Activity.class);
	...
	public Activity update(Activity activity) {
		...
		Response response = target.path("/activities/activity/"+activity.getId()).request(MediaType.APPLICATION_JSON)
